...
SGD_ASGD_process <- function(data,f,alpha,batch,epoches,
  ...
  ASGD_P = ASGD_Q = ASGD_bu = ASGD_bi = ASGD_xj = 0
  ...
  for(i in 1:epoches){
    ...
    for(j in seq(1,nrow(data),batch)){
      ...
      pred = mu + colSums(SGD_Q[,batch_item] * t(t(SGD_P[,batch_user]) + 
      ...
      #厩策瞯癐搭
      count = count+1
      new_lr_p = min(3,lr_p*(count^-alpha))
      new_lr_q = min(3,lr_q*(count^-alpha))
      new_lr_xj = min(1,lr_xj*(count^-alpha))
  ...
  SGD <- list(SGD_P,SGD_Q,SGD_bu,SGD_bi,SGD_xj,mu)
  ASGD <- list(ASGD_P,ASGD_Q,ASGD_bu,ASGD_bi,ASGD_xj,mu)
...
Ru <- matrix(NA,nrow=nrow(user),ncol=nrow(item))
for(i in unique(train$user)){
  Ru[user[as.character(i),],item[as.character(train$item[train$user==i]),]] <- T
}
Ru[is.na(Ru)] <- F

len_Ru <- rowSums(Ru)^(-1/2)
sum_xj <- rowSums(Ru %*% t(BatchSGD[[5]]))


y = BatchSGD[[6]] + t(BatchSGD[[2]]) %*% t(t(BatchSGD[[1]]) + len_Ru*sum_xj)  +
  matrix(rep(BatchSGD[[3]],nrow(item)),nrow = nrow(item),byrow=T) + matrix(rep(BatchSGD[[4]],nrow(user)),ncol = nrow(user))


train['pred'] <- NA
for(i in 1:nrow(train)){
  train[i,'pred'] <- y[item[as.character(train[i,'item']),],user[as.character(train[i,'user']),]]
}
...
for(i in 1:nrow(test)){
  test[i,'pred'] <- y[item[as.character(test[i,'item']),],user[as.character(test[i,'user']),]]
}
#RMSE 
#RMSE 
