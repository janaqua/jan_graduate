...
#圖4.1.1.11 學習率調整後前1000次更新ASGD的估計變異矩陣RMSE

matplot(1:1000,asgd_err[1:1000,1],type='l',main='finite sample: ASGD',ylab='RMSE',xlab='iteration t')

#圖4.1.1.12 學習率調整後後24000次更新ASGD的估計變異矩陣RMSE

matplot(1:24000,asgd_err[1001:25000,],type='l',main='finite sample: ASGD',ylab='RMSE',xlab='iteration t',xaxt='n')
...
#圖4.1.1.13 學習率調整後前1000次更新ASGD的估計變異矩陣trace
...
matplot(1:1000,asgd_trV[1:1000,1],type='l',main='finite sample: ASGD trace',ylab='trace',xlab='iteration t')
abline(h=sum(diag(compare)),lwd=2,lty=1,col=1)
text(x = 200,y=sum(diag(compare))+2,labels = 'trace(V)')

#圖4.1.1.14 學習率調整後後24000次更新ASGD的估計變異矩陣trace

matplot(1:24000,asgd_trV[1001:25000,],type='l',main='finite sample: ASGD trace',ylab='trace',xlab='iteration t',xaxt='n')
axis(1,at = c(seq(0,24000,5000),24000),labels = c(1000,seq(5000,25000,5000)))
legend('topright',lwd=1.5,lty=1:6,col = 1:6,legend = paste('alpha',alpha,sep= '='),cex = 1.2)
abline(h=sum(diag(compare)),lwd=2,lty=1,col=1)
