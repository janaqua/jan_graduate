...
result_all_old <-cbind(SGD_final,SGD_decay_final,ASGD_decay_final,
                                 ...
                                 lamb_w,lamb_t,lamb_beta,alpha,epoches,batch)
{
  ...
  for(i in 1:epoches){
    ...
    for(j in seq(1,N,batch)){
      ...
      #gamma_Decay
      lr_w = min(first_lr_w,(first_lr_w*100^alpha)*(n^-alpha))
      lr_t = min(first_lr_t,(first_lr_t*100^alpha)*(n^-alpha))
      lr_beta = min(first_lr_beta,(first_lr_beta*100^alpha)*(n^-alpha))
      n = n+1
...
#SGD
lr_w = 0.18 ; lr_t = 0.2 ; lr_beta = 0.2 #learnging rate gamma
lamb_w = 0. ; lamb_t = 0. ; lamb_beta = c(0.,0.,0.)  # regularize coefficient
alpha = 0. #alpha = 0 means no decay
epoches = 50
batch = 1 #batch = 1 means ASGD & SGD and others mean batch
...
#SGD ASGD with gamma decay
lr_w = 2.1 ; lr_t = 2.8 ; lr_beta = 1.11 #learnging rate gamma
lamb_w = 0. ; lamb_t = 0. ; lamb_beta = c(0.,0.,0.)  # regularize coefficient
alpha = 0.6 #alpha = 0 means no decay
epoches = 50
batch = 1 #batch = 1 means ASGD & SGD and others mean batch
...
result_all_new <- cbind(SGD_final,SGD_decay_final,ASGD_decay_final,
                        Batch_SGD_final,Batch_SGD_decay_final,Batch_ASGD_decay_final)

#圖4.1.3.2各式梯度下降法在不同epoch下之RMSE
matplot(1:50,cbind(SGD_final,SGD_decay_final,ASGD_decay_final,
                   Batch_SGD_final,Batch_SGD_decay_final,Batch_ASGD_decay_final),
        ylab = 'RMSE',xlab='epoch',type='l',ylim=c(0,0.8))
legend('topright',legend = c('SGD','SGD_decay','ASGD_decay','BatchSGD',
...
#圖4.1.3.3
matplot(1:50,cbind(result_all_old[,c(2,3,5,6)],result_all_new[,c(2,3,5,6)]),
        ylab = 'RMSE',xlab='epoch',type='l',col = 1:8,lwd = 1,ylim=c(0,0.2))
legend('topright',legend = c('no adjust SGD_decay','no adjust ASGD_decay',
                             'no adjust Batch_SGD_decay','no adjust Batch_ASGD_decay',
                             'adjust SGD_decay','adjust ASGD_decay',
                             'adjust Batch_SGD_decay','adjust Batch_ASGD_decay'),col=1:8,lty = 1:8)


#圖4.1.3.4各式梯度下降法累積訓練時間(second)
