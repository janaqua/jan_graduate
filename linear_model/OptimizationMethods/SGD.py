...
class ord_reg():
    ...
    def train(self,
        ...
        for i in range(epoches):
            ...
            for n, record in enumerate(self.data, 1):

                x_arr = np.expand_dims(record[:-1], 0)
                rating = np.expand_dims(record[-1], 0).astype(int)
            ...
            #Gamma Decay
                ...
                if self.count==1 :
                    self.get_before_error_r = self.data.copy()[:n+1,-1]                   
                    self.count+=1
    ...
    def evaluate(self, test_data): #calculate test_data's RMSE
