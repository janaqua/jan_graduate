...
SGD_ASGD_gamma_decay <- function(data,lr_w,lr_t,lr_beta,
  ...
  return(list(w_hat_list,t_hat_list,beta_hat_list,as.numeric(difftime(Sys.time(),time_start,units = 'secs'))))
}

lr_w = 0.2 ; lr_t = 0.2 ; lr_beta = c(0.18,0.02,0.023) #learnging rate gamma
...
#RMSE counting
SGD_RMSE_w <- sqrt(mean((tail(result[[1]],1) - param[[1]])^2))
SGD_RMSE_t <- sqrt(mean((tail(result[[2]],1) - param[[2]])^2))
SGD_RMSE_beta <- sqrt(mean((tail(result[[3]],1) - param[[3]])^2))

ASGD_RMSE_w <- sqrt(mean((colMeans(result[[1]]) - param[[1]])^2))
ASGD_RMSE_t <- sqrt(mean((colMeans(result[[2]]) - param[[2]])^2))
ASGD_RMSE_beta <- sqrt(mean((colMeans(result[[3]]) - param[[3]])^2))

RMSE_SGD <- c(SGD_RMSE_w,SGD_RMSE_t,SGD_RMSE_beta)
RMSE_ASGD <- c(ASGD_RMSE_w,ASGD_RMSE_t,ASGD_RMSE_beta)

RMSE <- rbind(RMSE_SGD,RMSE_ASGD)
RMSE <- cbind(RMSE,rowMeans(RMSE[,1:3]))
RMSE <- cbind(RMSE,result[[4]])
colnames(RMSE) <- c('w','t','beta','overall','training time')
RMSE
